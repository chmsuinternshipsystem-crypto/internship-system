<x-app-layout>
    ...
    <div
        ...
        x-on:close-review-panel.window="reviewOpen = false; htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#queue-table-wrapper', swap: 'innerHTML'})"
        x-on:reload-queue.window="htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#queue-table-wrapper', swap: 'innerHTML'})"
    ...
    >
        ...
        <div
            ...
            @keydown.escape.window="if (reviewOpen) { reviewOpen = false; htmx.ajax('GET', window.location.pathname + window.location.search, {target: '#queue-table-wrapper', swap: 'innerHTML'}); }"
        >
    ...
    @push('scripts')
        <script>
            // Bridge HX-Trigger events dispatched on body to window (so Alpine .window handlers can catch them)
            document.body.addEventListener('close-review-panel', function () {
                window.dispatchEvent(new CustomEvent('close-review-panel'));
            });
            document.body.addEventListener('reload-queue', function () {
