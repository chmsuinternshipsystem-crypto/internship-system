                    ...
                    @forelse ($industries as $industry)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right text-sm font-medium cell-tight whitespace-nowrap">
                                <a href="{{ route('company-industries.edit', $industry) }}" class="text-emerald-600 hover:text-emerald-800 mr-3">
                                    <i class="bi bi-pencil"></i> {{ __('Edit') }}
                                </a>
                                <button type="button"
                                        @click="if(confirm('{{ __('Delete this industry? Companies under it will lose their classification.') }}')) document.getElementById('delete-industry-{{ $industry->id }}').submit()"
                                        class="text-red-500 hover:text-red-700">
                                    <i class="bi bi-trash"></i> {{ __('Delete') }}
                                </button>
                                <form id="delete-industry-{{ $industry->id }}" action="{{ route('company-industries.destroy', $industry) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
