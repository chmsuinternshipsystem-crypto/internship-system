                ...
                <div class="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                    ...
                    @else
                        ...
                        <div class="space-y-1">
                            ...
                            <a href="{{ route('student.announcements') }}" class="block px-2 py-2.5 text-sm {{ request()->routeIs('student.announcements*') ? 'font-semibold text-emerald-700' : 'text-gray-700' }}">{{ __('Announcements') }}</a>
                            <a href="{{ route('student.messages.index') }}" class="block px-2 py-2.5 text-sm {{ request()->routeIs('student.messages*') ? 'font-semibold text-emerald-700' : 'text-gray-700' }}">
                                {{ __('Messages') }}
                                        <span x-data="{ count: 0 }"
                                               x-init="window.addEventListener('refresh-messages', async () => { try { let r = await fetch('{{ route('student.messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }); setInterval(async () => { try { let r = await fetch('{{ route('student.messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }, 10000)"
                                               x-show="count > 0"
                                               class="ml-1.5 inline-flex items-center justify-center w-2 h-2 rounded-full bg-red-500 shrink-0"
                    ...
                    @if ($role !== 'student')
                    <div class="space-y-1">
                        <p class="px-2 pt-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ __('Administration') }}</p>
                        <a href="{{ route('archive.index') }}" class="block px-2 py-2.5 text-sm {{ request()->routeIs('archive.*') ? 'font-semibold text-emerald-700' : 'text-gray-700' }}"><i class="bi bi-archive me-1.5"></i>{{ __('Archive') }}</a>
                    </div>
                    @endif
                </div>
                ...
                <div class="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                        ...
                        <div class="space-y-1">
                            ...
                            @if ($showWorkflowQueueNav)
                                <a href="{{ route('student-documents.queue') }}" class="block px-2 py-2.5 text-sm {{ request()->routeIs('student-documents.queue') ? 'font-semibold text-emerald-700' : 'text-gray-700' }}">
                                    {{ __('Document Queue') }}
                                    @if (!$onQueuePage)
                                    <span x-data="{ count: {{ $pendingQueueCount }} }"
                                          x-init="setInterval(async () => { try { let r = await fetch('{{ route('student-documents.queue.count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }, 10000)"
                                          x-show="count > 0"
                                          x-text="count"
                                          class="ml-1.5 inline-flex items-center justify-center min-w-[1.25rem] h-5 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold leading-none"
                                          style="display: {{ $pendingQueueCount > 0 ? 'inline-flex' : 'none' }}"></span>
                                    @endif
                                </a>
                            @endif
                        </div>
                    ...
                    @if ($showStaffCommSection)
                        <div class="space-y-1">
                            ...
                            @endif
                            @if ($showMessageNav)
                                <a href="{{ route('messages.index') }}" class="block px-2 py-2.5 text-sm {{ request()->routeIs('messages.*') ? 'font-semibold text-emerald-700' : 'text-gray-700' }}">
                                    {{ __('Messages') }}
                                     <span x-data="{ count: 0 }"
                                            x-init="window.addEventListener('refresh-messages', async () => { try { let r = await fetch('{{ route('messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }); setInterval(async () => { try { let r = await fetch('{{ route('messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }, 10000)"
                                            x-show="count > 0"
                                            class="ml-1.5 inline-flex items-center justify-center w-2 h-2 rounded-full bg-red-500 shrink-0"
                                            style="display: none"></span>
                                </a>
    ...
    <div class="hidden md:flex md:flex-col h-full">
        ...
        <div class="flex-1 px-0 py-3 overflow-y-auto">
            ...
            @if ($role === 'student')
                        ...
                        <a href="{{ route('student.messages.index') }}" class="nav-link {{ request()->routeIs('student.messages*') ? 'active' : '' }}">
                            <i class="bi bi-chat-dots me-2"></i>{{ __('Messages') }}
                                  <span x-data="{ count: 0 }"
                                        x-init="window.addEventListener('refresh-messages', async () => { try { let r = await fetch('{{ route('student.messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }); setInterval(async () => { try { let r = await fetch('{{ route('student.messages.unread-count') }}'); let d = await r.json(); count = d.count; } catch(e) {} }, 10000)"
                                        x-show="count > 0"
                                        class="ml-auto inline-flex items-center justify-center w-2 h-2 rounded-full bg-red-500 shrink-0"
                                        style="display: none"></span>
                             </a>
                         </div>
                     @endif
                ...
                @if ($role !== 'student')
                <div class="space-y-1">
                    <p class="px-4 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ __('Administration') }}</p>
                    <a href="{{ route('archive.index') }}" class="nav-link {{ request()->routeIs('archive.*') ? 'active' : '' }}"><i class="bi bi-archive me-2"></i>{{ __('Archive') }}</a>
                </div>
                @endif
            @endif
        </div>
