...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-50">
        ...
        <div class="h-screen flex overflow-hidden">
            @if (! $panelMode)
                <aside class="hidden md:flex md:flex-col w-64 bg-white border-r border-gray-200 h-full shadow-sm">
            ...
            <div class="flex-1 flex flex-col h-full overflow-hidden bg-gray-50">
                ...
                @isset($header)
                    <header class="bg-white/95 backdrop-blur-md border-b border-gray-100 flex-shrink-0 sticky top-0 z-20">
                        <div class="py-3 px-4 sm:px-6 lg:px-8 flex items-start justify-between gap-3">
                            ...
                            @if (auth()->check() || session()->has('student_account_id'))
                                <div class="flex items-center gap-2 shrink-0 pt-0.5">
                                    <button type="button" onclick="window.dispatchEvent(new CustomEvent('open-palette'))" class="hidden sm:inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-500 hover:border-emerald-300 hover:text-emerald-700 hover:bg-emerald-50 transition-colors" title="{{ __('Search') }}">
                                        <i class="bi bi-search"></i>
                                        <span class="hidden lg:inline">{{ __('Search') }}</span>
                                        <kbd class="hidden xl:inline-flex items-center rounded border border-gray-200 bg-gray-50 px-1 py-0.5 text-[10px] font-medium text-gray-400">/</kbd>
                                    </button>
        ...
        @auth
            <x-command-palette />
            <x-fab-menu role="{{ auth()->user()?->role ?? '' }}" />
        @endauth
        ...
        <script>
            ...
            // Global HTMX error handler — shows meaningful messages, not generic blanket toasts
            document.addEventListener('htmx:responseError', function (e) {
                if (typeof hideLoadingOverlay === 'function') hideLoadingOverlay();
                var xhr = e.detail.xhr;
                var ct = (xhr.getResponseHeader('Content-Type') || '');
                // If response is HTML, the error is already rendered inline in the swapped content
                ...
                if (xhr.status === 413) {
                    msg = '{{ __('File too large. Maximum upload size is 2MB for documents.') }}';
                } else if (xhr.status === 429) {
                    msg = '{{ __('Too many requests. Please wait a moment and try again.') }}';
                } else if (xhr.status === 419) {
                    return;
                } else if (xhr.status === 403) {
                    msg = '{{ __('You do not have permission to perform this action.') }}';
                } else if (xhr.status === 404) {
                    msg = '{{ __('The requested resource was not found.') }}';
                } else if (xhr.status >= 500) {
                    msg = '{{ __('A server error occurred. Please try again or contact your coordinator.') }}';
                }
            ...
            @if ($isStudentRoute && (request()->attributes->get('studentPortalLimited') ?? false))
                (function () {
                    ...
                    pollPortalAccess();
                    setInterval(pollPortalAccess, 10000);
                })();
            @endif
        </script>
    </body>
