...
<x-app-layout>
    ...
    <div class="py-12"
         x-data="{
             ...
             init() {
                 ...
                 window.addEventListener('refresh-documents', () => { if (this.tab === 'documents') this.loadTab('documents'); });
                 window.addEventListener('refresh-journals', () => { if (this.tab === 'journals') this.loadTab('journals'); });
                 window.addEventListener('refresh-attendance', () => { if (this.tab === 'attendance') this.loadTab('attendance'); });
                 window.addEventListener('refresh-certificates', () => { if (this.tab === 'certificates') this.loadTab('certificates'); });
                 window.addEventListener('refresh-dtr', () => { if (this.tab === 'dtr') this.loadTab('dtr'); });
             },
              loadTab(val) {
                  ...
                  if (val === 'profile') {
                      fetch('{{ route('students.tab.profile', $student) }}')
                          .then(r => r.text())
                          .then(html => { if (version === this.loadedTabs[val]) document.getElementById('profile-tab-content').innerHTML = html; })
                          .catch(() => { if (version === this.loadedTabs[val]) document.getElementById('profile-tab-content').textContent = 'Failed to load profile.'; });
                  } else if (val === 'documents') {
                      fetch('{{ route('students.tab.documents', $student) }}')
                          .then(r => r.text())
                          .then(html => { if (version === this.loadedTabs[val]) document.getElementById('documents-tab-content').innerHTML = html; })
                          .catch(() => { if (version === this.loadedTabs[val]) document.getElementById('documents-tab-content').textContent = 'Failed to load documents.'; });
                  } else if (val === 'journals') {
                      if (window.htmx) {
                      ...
                      fetch('{{ route('students.tab.dtr', $student) }}')
                          .then(r => r.text())
                          .then(html => { if (version === this.loadedTabs[val]) { document.getElementById('dtr-tab-content').innerHTML = html; if (window.htmx) htmx.process(document.getElementById('dtr-tab-content')); } })
                      ...
                      fetch('{{ route('students.tab.attendance', $student) }}')
                          .then(r => r.text())
                          .then(html => { if (version === this.loadedTabs[val]) { document.getElementById('attendance-tab-content').innerHTML = html; if (window.htmx) htmx.process(document.getElementById('attendance-tab-content')); } })
    ...
    >
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Tab content — lazy-loaded via fetch/HTMX --}}
            <div>
                <div x-show="tab === 'profile'" x-cloak role="tabpanel">
                    <div id="profile-tab-content" class="py-8 text-center text-gray-400">
                        <i class="bi bi-arrow-repeat text-2xl animate-spin inline-block"></i>
                        <p class="mt-2 text-sm">{{ __('Loading profile...') }}</p>
                    </div>
                </div>
                <div x-show="tab === 'documents'" x-cloak role="tabpanel">
                    <div id="documents-tab-content" class="py-8 text-center text-gray-400">
                        <i class="bi bi-arrow-repeat text-2xl animate-spin inline-block"></i>
                        <p class="mt-2 text-sm">{{ __('Loading documents...') }}</p>
                    </div>
                </div>
