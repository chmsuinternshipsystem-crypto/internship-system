...
<div id="{{ $cardId }}" x-data='{ activityOpen: false, previewOpen: false, activities: @json($cardActions), fileSelected: false, fileName: "", fileError: "" }'
    class="grid sm:grid-cols-12 gap-3 items-center px-4 py-3 rounded-lg border border-gray-100 bg-white hover:bg-gray-50 transition-colors">
    <div class="sm:col-span-5 flex items-center gap-2 min-w-0">
        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-50 ring-1 ring-gray-200 text-gray-500">
            <i class="bi bi-file-text text-sm"></i>
        </div>
        <div class="min-w-0">
            <p class="text-sm font-medium text-gray-900 truncate">{{ $doc->name }}</p>
            @if ($doc->company)
                <p class="text-xs text-indigo-600 truncate">{{ $doc->company->name }}</p>
            @endif
            @php $cardTemplate = $doc->relationLoaded('template') ? $doc->template : $doc->template()->first(); @endphp
            @if ($cardTemplate)
                <a href="{{ route('required-documents.template.download', $doc) }}" class="inline-flex items-center gap-1 text-[11px] text-emerald-600 hover:text-emerald-800 font-medium mt-0.5">
    ...
    <div class="sm:col-span-2">
        @php
            $phaseColors = [
                'pre' => 'bg-blue-50 text-blue-700 ring-blue-200',
                'monitoring' => 'bg-amber-50 text-amber-700 ring-amber-200',
                'post' => 'bg-emerald-50 text-emerald-700 ring-emerald-200',
            ];
            $phaseColor = $phaseColors[$doc->phase] ?? 'bg-gray-50 text-gray-700 ring-gray-200';
            $phaseLabel = match ($doc->phase) {
                'pre' => __('Pre'),
                'monitoring' => __('Monitoring'),
                'post' => __('Post'),
                default => ucfirst($doc->phase),
            };
        @endphp
        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 {{ $phaseColor }}">{{ $phaseLabel }}</span>
    </div>
    <div class="sm:col-span-3">
        <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold {{ $statusClass }}">
            <i class="bi {{ $statusIcon }}"></i>
            {{ $statusText }}
        </span>
        @if ($isOverdue)
            <span class="ml-1 text-[11px] text-rose-600 font-medium">{{ __('Overdue') }}</span>
        @elseif ($isDueToday)
            <span class="ml-1 text-[11px] text-amber-600 font-medium">{{ __('Due today') }}</span>
        @endif
    </div>
    <div class="sm:col-span-2 text-right">
        @if (! $current || ($wf !== 'completed' && $dbStatus !== 'Submitted'))
            <form method="POST" action="{{ route('student.documents.upload', $doc) }}" enctype="multipart/form-data"
                ...
                @csrf
                <input type="file" name="file" required accept=".pdf,.doc,.docx" class="w-28 text-xs file:mr-1 file:rounded-md file:border-0 file:bg-emerald-50 file:px-2 file:py-1 file:text-[11px] file:font-semibold file:text-emerald-700 hover:file:bg-emerald-100">
                <button type="submit" class="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-2.5 py-1.5 text-[11px] font-semibold text-white transition hover:bg-emerald-700">
        ...
        @endif
        @if ($actionCount > 0)
            <button type="button" @click="activityOpen = true" class="ml-1 text-xs text-gray-500 hover:text-gray-700">
...
<div id="{{ $cardId }}" x-data='{ activityOpen: false, previewOpen: false, activities: @json($cardActions), fileSelected: false, fileName: "", fileError: "" }'
    ...
    <div class="p-4 sm:p-5">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
            ...
            @php $cardTemplate = $doc->relationLoaded('template') ? $doc->template : $doc->template()->first(); @endphp
            @if ($cardTemplate)
                <a href="{{ route('required-documents.template.download', $doc) }}" class="inline-flex items-center gap-1.5 text-emerald-600 hover:text-emerald-800 font-medium">
                    <i class="bi bi-download"></i>
                    <span class="truncate max-w-[120px]">{{ $cardTemplate->original_name }}</span>
                </a>
            @endif
        </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-3">
            ...
            <form method="POST" action="{{ route('student.documents.upload', $doc) }}" enctype="multipart/form-data"
                ...
                <input type="file" name="file" required accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    @change="fileError = ''; const f = $event.target.files[0]; if (f && f.size > 2097152) { fileError = '{{ __('File exceeds 2MB limit.') }}'; $event.target.value = ''; fileSelected = false; fileName = ''; } else { fileSelected = !!f; fileName = f ? f.name : ''; }"
                ...
                <template x-if="fileError">
                    <span class="w-full text-xs text-rose-600 font-medium" x-text="fileError"></span>
                </template>
            </form>
            @else
            <span class="inline-flex items-center gap-1.5 text-xs text-emerald-600 font-medium">
                <i class="bi bi-check-circle"></i>
                {{ __('Document verified — no further action needed') }}
            </span>
            @endif
            ...
            <div x-show="previewOpen"
                ...
                <div x-show="previewOpen"
                    ...
                    <div class="flex-1 flex overflow-hidden">
                        @if ($isPdf)
                            <div class="flex-1 flex flex-col" x-data="pdfPreview(@js($downloadUrl))">
