<x-app-layout>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-700 via-emerald-600 to-emerald-500 p-6 sm:p-8 shadow-lg mb-6">
        <div class="absolute inset-0 opacity-[0.03]" style="background-image: radial-gradient(circle at 20% 30%, white 1px, transparent 1px), radial-gradient(circle at 80% 70%, white 1px, transparent 1px); background-size: 40px 40px, 30px 30px;"></div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <x-page-card compact>
            <p class="text-xs uppercase tracking-wide text-gray-500 mb-3">{{ __('OJT Grade') }}</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50/50 px-3 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600">{{ __('Company Supervisor') }}</p>
                            @if ($ojtGrade['hte_score'] !== null)
                                <p class="text-lg font-bold text-blue-700 tabular-nums">{{ $ojtGrade['hte_score'] }}<span class="text-xs font-normal text-gray-400">/100</span></p>
                                <p class="text-[11px] text-gray-500">{{ __('Rated by industry supervisor') }}</p>
                            @else
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50/50 px-3 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600">{{ __('OJT Instructor') }}</p>
                            @if ($ojtGrade['instructor_score'] !== null)
                                <p class="text-lg font-bold text-emerald-700 tabular-nums">{{ $ojtGrade['instructor_score'] }}<span class="text-xs font-normal text-gray-400">/100</span></p>
                                <p class="text-[11px] text-gray-500">{{ __('Rated by OJT coordinator') }}</p>
                            @else
                                <p class="text-sm text-gray-500">{{ __('Pending') }}</p>
                            @endif
                        </div>
            ...
            @if ($ojtGrade['is_complete'])
                <div class="mt-3 flex items-center justify-between rounded-lg border border-emerald-100 bg-emerald-50/50 px-3 py-2">
                    <div class="flex items-center gap-2">
                        <i class="bi bi-trophy text-emerald-600 text-sm"></i>
                        <span class="text-xs font-semibold text-gray-700">{{ __('Final Grade') }}</span>
                    </div>
                    <span class="text-lg font-bold text-emerald-700 tabular-nums">{{ number_format((float) $ojtGrade['final_grade'], 2) }}<span class="text-xs font-normal text-gray-400">/100</span></span>
                </div>
            @else
                <p class="mt-2 text-xs text-gray-400 text-center">{{ __('Awaiting both evaluations to compute final grade') }}</p>
            @endif
        </x-page-card>
    </div>
            ...
            <div class="space-y-2">
                @forelse ($announcements as $announcement)
                    <div class="rounded-lg border border-gray-200 px-3 py-2.5 hover:border-emerald-200 transition-colors">
                        ...
                        <p class="text-sm text-gray-700 mt-1.5 line-clamp-3">{{ $announcement->body }}</p>
                        @if (strlen(strip_tags((string) $announcement->body)) > 200)
                            <a href="{{ route('student.announcements') }}" class="text-xs font-medium text-emerald-600 hover:text-emerald-800 mt-0.5 inline-block">{{ __('Read more') }}</a>
                        @endif
                    </div>
                @empty
