                        ...
                        <div x-data="{
                            ...
                            init() {
                                ...
                                document.addEventListener('htmx:afterSwap', (e) => {
                                    if (e.detail.target.id === 'assignable-list-mount') {
                                        this.refreshAllIds();
                                    }
                                });
                            },
                            refreshAllIds() {
                                this.allIds = Array.from(
                                    document.querySelectorAll('#assignable-list-mount input[name=\'student_ids[]\']')
                            ...
                            <div class="flex flex-wrap items-center gap-2 mb-3">
                                <div class="relative flex-1 min-w-[180px]">
                                   ...
                                   @input.debounce.300ms="if (window.htmx) { htmx.ajax('GET', '{{ route('companies.students.assignable', $company) }}?search=' + encodeURIComponent(search) + '&my_students=' + (myStudents ? '1' : '0'), { target: '#assignable-list-mount', swap: 'innerHTML' }); }"
                                ...
                                <label class="inline-flex items-center gap-1.5 cursor-pointer text-xs text-gray-600 shrink-0">
                                   ...
                                   @change="if (window.htmx) { htmx.ajax('GET', '{{ route('companies.students.assignable', $company) }}?search=' + encodeURIComponent(search) + '&my_students=' + (myStudents ? '1' : '0'), { target: '#assignable-list-mount', swap: 'innerHTML' }); }"
