        ...
        <x-page-card compact class="h-full">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input id="contact_phone" name="contact_phone" type="text" inputmode="tel"                                maxlength="16"
        ...
        <x-page-card compact class="h-full">
            ...
            <div class="space-y-2"
                ...
                <div class="flex items-center justify-between gap-2">
                    <span class="text-xs text-gray-400">{{ __('Click the map or enter coordinates manually.') }}</span>
                </div>
            </div>
        </x-page-card>
    </div>
    ...
    <script>
        ...
        function addressCascader(initial) {
            return {
                ...
                barangayNameText: '',
                async init() {
                    const savedProvince = String(initial.provinceId || '');
                    const savedCity = String(initial.cityId || '');
                    const savedBarangay = String(initial.barangayId || '');

                    const res = await fetch('{{ route('address.provinces') }}');
                    this.provinces = await res.json();

                    if (savedProvince) {
                        this.provinceId = savedProvince;
                        const cityRes = await fetch('{{ route('address.cities', ['province' => '__PROVINCE__']) }}'.replace('__PROVINCE__', savedProvince));
                        this.cities = await cityRes.json();

                        if (savedCity) {
                            this.cityId = savedCity;
                            const barRes = await fetch('{{ route('address.barangays', ['city' => '__CITY__']) }}'.replace('__CITY__', savedCity));
                            this.barangays = await barRes.json();

                            if (savedBarangay) {
                                this.barangayId = savedBarangay;
                                const selected = this.barangays.find(b => String(b.id) === savedBarangay);
                                this.barangayNameText = selected ? selected.name : '';
                            }
                        }

                        const citySelected = this.cities.find(c => String(c.id) === savedCity);
                        this.cityNameText = citySelected ? citySelected.name : '';
                    }
                },
