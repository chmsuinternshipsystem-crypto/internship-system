...
class SampleDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $docNbiClearance = RequiredDocument::updateOrCreate(
            ['name' => 'NBI Clearance'],
            ['description' => 'NBI clearance (if applicable to the student)', 'is_mandatory' => false, 'order_index' => 10, 'phase' => 'monitoring']
        );
        ...
        RequiredDocument::updateOrCreate(
            ['name' => 'Signed DTTR'],
            ['description' => 'Signed Daily Time & Tasks Record for the entire internship period', 'is_mandatory' => true, 'order_index' => 13, 'phase' => 'post']
        );
        RequiredDocument::updateOrCreate(
            ['name' => 'Signed Weekly Journal'],
            ['description' => 'Signed and compiled weekly journals for the entire internship period', 'is_mandatory' => true, 'order_index' => 14, 'phase' => 'post']
        );

        // --- Student documents (create BEFORE deployments so saving event sees approved docs) ---
        $preDocIds = RequiredDocument::where('is_mandatory', true)->where('phase', 'pre')->pluck('id');
        $monitoringDocIds = RequiredDocument::where('is_mandatory', true)->where('phase', 'monitoring')->pluck('id');
        $postDocIds = RequiredDocument::where('is_mandatory', true)->where('phase', 'post')->pluck('id');

        foreach ([$s1, $s3, $s4] as $student) {
            foreach ($preDocIds as $docId) {
                StudentDocument::updateOrCreate(
                    ['student_id' => $student->id, 'required_document_id' => $docId],
                    ['status' => 'Submitted', 'submitted_at' => now()]
                );
            }
        }

        // s1: all monitoring + post docs submitted = fully complete
        foreach ($monitoringDocIds->merge($postDocIds) as $docId) {
            StudentDocument::updateOrCreate(
                ['student_id' => $s1->id, 'required_document_id' => $docId],
                ['status' => 'Submitted', 'submitted_at' => now()]
            );
        }

        // s3: monitoring docs submitted, no post docs yet
        foreach ($monitoringDocIds as $docId) {
            StudentDocument::updateOrCreate(
                ['student_id' => $s3->id, 'required_document_id' => $docId],
                ['status' => 'Submitted', 'submitted_at' => now()]
            );
        }

        // s4: all monitoring + post docs submitted = fully complete
        foreach ($monitoringDocIds->merge($postDocIds) as $docId) {
            StudentDocument::updateOrCreate(
                ['student_id' => $s4->id, 'required_document_id' => $docId],
                ['status' => 'Submitted', 'submitted_at' => now()]
            );
        }
        ...
        // --- Deployments (status auto-computed by saving event based on dates + doc compliance) ---
        Deployment::updateOrCreate(
            ...
            ['end_date' => '2026-08-28', 'remarks' => 'On track, good attendance.']
        );
        ...
        Deployment::updateOrCreate(
            ...
            ['end_date' => '2026-10-04', 'remarks' => 'Assigned to IT support team.']
        );
        ...
        Deployment::updateOrCreate(
            ...
            ['end_date' => '2025-11-30', 'remarks' => 'Completed with excellent evaluation.']
        );
