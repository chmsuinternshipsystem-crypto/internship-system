...
class ReportController extends Controller
{
    ...
    public function executiveSummary()
    {
        ...
        // Month-over-month trend: deployments this month vs last month
        $deploymentsThisMonth = Deployment::whereIn('status', ['active', 'completed'])
            ->whereMonth('start_date', now()->month)->whereYear('start_date', now()->year)->count();
        $deploymentsLastMonth = Deployment::whereIn('status', ['active', 'completed'])
            ->whereMonth('start_date', now()->subMonth()->month)->whereYear('start_date', now()->subMonth()->year)->count();
        $deploymentTrend = $deploymentsLastMonth > 0
            ? round((($deploymentsThisMonth - $deploymentsLastMonth) / $deploymentsLastMonth) * 100, 1)
            : ($deploymentsThisMonth > 0 ? 100 : 0);

        // Compliance trend
        $compliantCount = $compliantStudents;
        $prevMonthCompliant = $mandatoryDocs > 0
            ? Student::whereDoesntHave('documents', fn ($q) => $q
                ->whereIn('required_document_id', RequiredDocument::where('is_mandatory', true)->pluck('id'))
                ->where('status', '!=', 'Submitted')
                ->where('created_at', '<', now()->subMonth())
            )->count() : 0;
        $complianceTrend = $prevMonthCompliant > 0
            ? round((($compliantCount - $prevMonthCompliant) / $prevMonthCompliant) * 100, 1)
            : 0;
        ...
        return view('reports.executive-summary', compact(
            ...
            'deploymentTrend', 'complianceTrend',
        ));
    }
