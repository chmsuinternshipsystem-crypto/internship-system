...
class StudentDocumentController extends Controller
{
    ...
    public function queue(Request $request)
    {
        ...
        $myStudents = $request->has('my_students') ? $request->boolean('my_students') : (auth()->user()?->role === 'instructor');
        if ($myStudents) {
    ...
    public function update(Request $request, Student $student)
    {
        ...
        $justActivated = $student->autoActivateDeployment();
        ...
        if ($request->header('HX-Request')) {
            ...
            $response = response()->view('student-documents.partials.upload-panel', compact(
                'student',
                'updatedDoc',
            ));
            ...
            if ($justActivated) {
                $response->header('HX-Trigger', json_encode([
                    'close-review-panel' => true,
                    'refresh-dashboard' => time(),
                ]));
            }
        ...
        return redirect()
            ...
            ->with('status', __('Document compliance updated successfully.'));
    }
    ...
    public function workflowAction(Request $request, Student $student, StudentDocument $studentDocument)
    {
        ...
        $justActivated = false;
        if ($validated['action'] === 'approve' && $studentDocument->requiredDocument?->phase === 'pre') {
            $justActivated = $student->autoActivateDeployment();
        }
        ...
        if ($nextHolderRole !== '' && $nextHolderRole !== $beforeHolderRole && $nextHolderRole !== (string) auth()->user()->role) {
            $this->notificationService->notifyRole($nextHolderRole, [
                ...
                'action_url' => $nextHolderRole === 'chairperson'
        ...
        if ($request->header('HX-Request')) {
            ...
            $triggerData = ['close-review-panel' => true];
            if ($justActivated) {
                $triggerData['refresh-dashboard'] = time();
            }
            ...
            return response(
                ...
                200,
                [
                    'HX-Trigger' => json_encode($triggerData),
    ...
    public function review(Request $request, Student $student, StudentDocument $studentDocument)
    {
        ...
        $studentDocument->load([
            'requiredDocument:id,name,submission_deadline_at',
            'workflowTemplate.steps',
            'actions' => fn ($q) => $q->latest('acted_at')->limit(50),
            'actions.actor:id,name',
        ]);
