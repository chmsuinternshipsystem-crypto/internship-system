...
class WeeklyJournalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($myStudents) {
            $studentQuery->where('assigned_instructor_id', auth()->id());
        }

        // Only students with weekly journals (using WeeklyJournal query directly)
        $studentIdsWithJournals = WeeklyJournal::query()->select('student_id')->distinct()->pluck('student_id');
        $studentQuery->whereIn('id', $studentIdsWithJournals);

        $status = $request->query('status');
        $statusFilter = $status && in_array($status, ['draft', 'submitted', 'reviewed']) ? $status : null;

        $students = $studentQuery->orderBy('student_number')->paginate(5)->withQueryString();

        // Load journals for students on current page
        $studentIds = $students->pluck('id');
        $journals = WeeklyJournal::with(['deployment.company', 'reviewer'])
        ...
        $weeklyJournals = $journals;

        if (filter_var($request->header('HX-Request'), FILTER_VALIDATE_BOOLEAN)) {
            return view('weekly-journals.partials.ajax-list', compact('weeklyJournals', 'students', 'search', 'status', 'section', 'myStudents', 'statusFilter'));
        }

        return view('weekly-journals.index', compact('weeklyJournals', 'students', 'search', 'status', 'section', 'myStudents', 'statusFilter'));
    }
    ...
    public function updateActivities(Request $request, WeeklyJournal $weeklyJournal)
    {
        ...
        return response()->json(['saved' => true, 'message' => __('Journal saved.')]);
    }
    ...
    public function deleteFile(Request $request, WeeklyJournal $weeklyJournal)
    {
        ...
        return response()->json(['saved' => true, 'message' => __('File removed.')]);
    }
    ...
    private function ensureWeeksExist($student): void
    {
        ...
        if ($existingWeeks->isNotEmpty()) {
            ...
            $cursor = $lastWeekEnd->copy()->addDays(2);
        } else {
