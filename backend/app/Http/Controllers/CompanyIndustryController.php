...
class CompanyIndustryController extends Controller
{
    ...
    {
        $companiesCount = $companyIndustry->companies()->count();

        if ($companiesCount > 0) {
            return redirect()
                ->route('company-industries.index')
                ->with('status', __('Cannot delete: :count company(ies) are classified under this industry. Reassign them first.', ['count' => $companiesCount]))
        ...
        return redirect()
            ->route('company-industries.index')
            ->with('status', __('Industry deleted successfully.'))
            ->with('status_type', 'success');
    }
}
