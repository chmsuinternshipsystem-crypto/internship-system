...
class AnnouncementController extends Controller
{
    ...
    public function store(StoreAnnouncementRequest $request)
    {
        ...
        $body = Str::limit(strip_tags((string) $announcement->body), 200);
        $actionUrl = route('announcements.show', $announcement);

        $payload = [
            'event_type' => 'announcement.new',
            'title' => __('New announcement: :title', ['title' => $title]),
            'body' => $body,
            'action_url' => $actionUrl,
            'meta' => ['announcement_id' => (int) $announcement->id],
        ];

        // Notify all staff
        foreach (['instructor', 'chairperson', 'dean'] as $role) {
            $this->notificationService->notifyRole($role, $payload);
        }
        ...
        foreach ($studentAccounts as $studentAccount) {
            $this->notificationService->notifyStudentAccount($studentAccount, $payload);
        }
    ...
    {
        if (auth()->user() && ! auth()->user()->can('view', $announcement)) {
            return redirect()->route('announcements.index')
                ->with('status', __('This announcement is not available for your role.'))
