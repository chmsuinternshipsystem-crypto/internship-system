...
class MessageThreadController extends Controller
{
    ...
    public function show(Request $request, MessageThread $message)
    {
        ...
        return redirect()->route($indexRoute, ['conversation' => $message->id]);
    }
    ...
    public function toggleRead(Request $request, MessageThread $message)
    {
        ...
        return response(view('messages.partials.inbox-column', $data))
            ...
            ->header('HX-Trigger', json_encode(['refresh-inbox' => time()]));
    }
    ...
    public function unreadCount(Request $request): \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
    {
        // If a browser navigated here directly, redirect to messages page instead of showing raw JSON
        if (! $request->expectsJson() && ! $request->header('HX-Request')) {
            $route = MessageActor::isStudentPortal($request) ? 'student.messages.index' : 'messages.index';
