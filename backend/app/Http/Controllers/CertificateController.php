...
class CertificateController extends Controller
{
    ...
    public function show(Certificate $certificate)
    {
        ...
        if ($certificate->file_path && Storage::disk('public')->exists($certificate->file_path)) {
            $previewUrl = Storage::disk('public')->url($certificate->file_path);
        }

        return view('certificates.show', compact('certificate', 'previewUrl'));
    }

    public function previewPdf(Certificate $certificate)
    {
        $certificate->load(['student', 'deployment.company']);

        if (! $certificate->file_path || ! Storage::disk('public')->exists($certificate->file_path)) {
            return redirect()->route('certificates.show', $certificate)
                ->with('status', __('Certificate PDF not yet generated.'))
                ->with('status_type', 'error');
        }

        return Storage::disk('public')->response($certificate->file_path, $certificate->title . '.pdf', [
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'inline; filename="' . $certificate->title . '.pdf"',
        ]);
    }
    ...
    public function regenerate(Certificate $certificate)
    {
        ...
        if ($certificate->status === 'verified') {
            return back()->with('status', __('Cannot regenerate an already verified certificate.'))
                ->with('status_type', 'error');
        }
        ...
        $path = $service->generate($certificate->student, $certificate);
        $certificate->update(['file_path' => $path, 'generated_at' => now()]);

        return redirect()->route('certificates.show', $certificate)
            ->with('status', __('Certificate regenerated successfully.'))
            ->with('status_type', 'success');
    }
