...
class HteTransactionController extends Controller
{
    ...
    public function submitEvaluation(Request $request, string $token)
    {
        ...
        try {
            $evaluation = null;
            DB::transaction(function () use ($link, $criteriaScores, $computedScore, $comment, &$evaluation): void {
        ...
        if ($assignedInstructor) {
            $this->notificationService->notifyUsers([$assignedInstructor->id], [
                ...
                'body' => __('The HTE supervisor of :student has submitted the evaluation.', ['student' => $student?->name ?? '']),
                'action_url' => $evaluation ? route('evaluations.show', $evaluation) : route('dashboard'),
    ...
    public function submitUpload(Request $request, string $token)
    {
        ...
        // Notify instructors that a document was uploaded via HTE link
        try {
            $student = \App\Models\Student::find($link->student_id);
            if ($student) {
