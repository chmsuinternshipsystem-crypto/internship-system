...
class StudentPortalController extends Controller
{
    ...
    public function documents()
    {
        ...
        $monitoringSubmitted = $student->areAllMonitoringDocsSubmitted();

        $requiredDocuments = RequiredDocument::cachedOrdered()
            ->filter(fn ($doc) => is_null($doc->company_id) || ($activeCompanyId && $doc->company_id === $activeCompanyId))
            ->when(! $preApproved, fn ($q) => $q->reject(fn ($doc) => in_array($doc->phase, ['monitoring', 'post'], true)))
            ->when($preApproved && ! $monitoringSubmitted, fn ($q) => $q->reject(fn ($doc) => $doc->phase === 'post'));
    ...
    public function uploadDocument(Request $request, RequiredDocument $requiredDocument)
    {
        ...
        return redirect()
            ...
            ->with('status', __('Document uploaded successfully. Waiting for verification.'));
    }
    ...
    public function updatePassword(Request $request)
    {
        ...
        return redirect()->route($home)->with('status', __('Password updated successfully. Welcome!'));
    }
    ...
    public function logout(Request $request)
    {
        ...
        return redirect('/')->with('status', __('You have been signed out.'))->with('status_type', 'info');
    }
}
