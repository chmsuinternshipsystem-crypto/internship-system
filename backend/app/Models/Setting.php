...
class Setting extends Model
{
    ...
    private const CAMPUS_CACHE_KEY = 'chmsu-talisay-internship-system-cache-campus_settings';

    /** Returns the single campus settings row, cached forever. Flushed on every update. */
    public static function campus(): self
    {
        return cache()->rememberForever(self::CAMPUS_CACHE_KEY, function () {
    ...
    /** Flush the campus settings cache. Called automatically after any setting update. */
    public static function flushCampusCache(): void
    {
        cache()->forget(self::CAMPUS_CACHE_KEY);
    }

    /** @deprecated Use {@see campus()} directly — it is now cached. */
    public static function campusCached(): self
    {
        return static::campus();
    }
