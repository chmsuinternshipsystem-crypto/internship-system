    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2">
            <div>
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Create Student Profile') }}</h2>
                <p class="text-sm text-gray-500">{{ __('Register a new BSIS student for internship tracking.') }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('students.store') }}" method="POST"
                          x-data="duplicateNameGuard()"
                          @submit.prevent="checkBeforeSubmit">
                        @include('students._form', ['student' => new \App\Models\Student(), 'submitLabel' => __('Create')])
                    </form>
            ...
            function duplicateNameGuard() {
                return {
                    ...
                    checkBeforeSubmit(e) {
                        ...
                        const url = '{{ route('students.check-duplicate-name') }}?first_name=' + encodeURIComponent(firstName) + '&last_name=' + encodeURIComponent(lastName) + '&middle_name=' + encodeURIComponent(middleName) + '&name_extension=' + encodeURIComponent(nameExtension);
                        fetch(url)
                            .then(r => r.json())
                            .then(data => {
                                if (data.duplicate) {
