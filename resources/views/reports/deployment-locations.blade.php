    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2">
            <div>
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Deployment Locations') }}</h2>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('reports.deployment-locations', ['export' => 'pdf']) }}" class="btn btn-sm btn-outline-success"><i class="bi bi-file-earmark-pdf me-1"></i>{{ __('Export PDF') }}</a>
            ...
            @foreach ($companies as $row)
                <x-page-card compact>
                    ...
                    @if ($row['company']->address)
                        <p class="text-xs text-gray-500 mb-3"><i class="bi bi-geo-alt me-1"></i>{{ $row['company']->address }}</p>
                    @endif
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($row['deployments'] as $dep)
                                        <tr>
                                            ...
                                            <td class="px-4 py-2 text-sm text-gray-600">{{ $dep->student?->section ?? '-' }}</td>
